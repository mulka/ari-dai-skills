...
### Fixed
- **`install.sh --all` now prunes stale symlinks.** Renaming or deleting a skill upstream (e.g. `context` → `graphs` in 1dc6e56) left a dangling symlink in every project that had installed it — `update.sh` re-linked the surviving skills but never removed the orphan, so each project accumulated broken links that agents would try to load. A full install now sweeps the install directory (`.claude/skills`, `.cursor/rules`, or `.github/instructions`) and removes broken symlinks whose target points back into a dai-skills `skills/` tree. Working links, real files, and broken links pointing anywhere else are left untouched, so hand-made skills and other tools' rules survive. Single-skill installs are unchanged. Covered by a new CI smoke test.
